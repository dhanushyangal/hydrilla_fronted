...
function LibraryCard({
  ...
  job: BackendJob;
  convertBackendStatus: (s: BackendJob["status"]) => JobStatus;
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white overflow-hidden hover:border-gray-300 transition-all group shadow-sm hover:shadow-md">
  ...
  return (
    <div className="space-y-6 sm:space-y-8 px-4 lg:px-8 py-4 sm:py-6 max-w-7xl mx-auto pt-20 sm:pt-24">
      ...
      <div>
          ...
          <p className="text-gray-600 mt-1 sm:mt-2 text-sm sm:text-base">Your 3D objects and generated images.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-10">
          {/* Section: 3D Objects */}
          {(() => {
            const objects3d = jobs.filter((j) => j.resultGlbUrl);
            return objects3d.length > 0 ? (
              <section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {objects3d.map((job) => (
                    <LibraryCard
                      key={job.id}
                      job={job}
                      convertBackendStatus={convertBackendStatus}
                      onView={(id) => `/viewer?jobId=${id}`}
          ...
          {/* Section: Images */}
          {(() => {
            const images = jobs.filter((j) => (j.previewImageUrl || j.imageUrl) && !j.resultGlbUrl);
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {images.map((job) => (
                    <LibraryCard
                      ...
                      job={job}
                      convertBackendStatus={convertBackendStatus}
                      onView={(id) => `/viewer?jobId=${id}&mode=image`}
                      onDelete={handleDeleteClick}
                      deletingId={deletingId}
          ...
          {/* Empty state when both sections would be empty (handled above by jobs.length > 0) */}
        </div>
      )}
